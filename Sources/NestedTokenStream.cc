...
void NestedTokenStream::newTokenStream(AtlasDLG *ats) {
    ...
    if (depth >= maxDepth) {
      ...
      newStack=new AtlasDLG *[newDepth];
      if (newStack == 0) {
        panic("NestedTokenStream can't allocated AtlasDLG stack");
      };
      if (stack != 0) {
...
NestedTokenStream::NestedTokenStream(AtlasDLG *ats) {
  init();
  newTokenStream(ats);
}
